    ...
    void Update()
    {
        ...
        if (IsOn)
        {
            ...
            if(LoadingBar.fillAmount >= 1.0)
            {
                ...
                barTime = 0.0f;
                LoadingBar.fillAmount = 0;
                CaptureCamera.GetComponent<cshCaptureCameraCNN>().CaptureHands();
                Invoke("captureInvoke", 1.0f);
                
            }
        }
    }
    private void captureInvoke()
    {
        CaptureCamera.GetComponent<cshInceptionv3ImageClassifierApplyCNN>().PredictedHands(false);
    }
}
